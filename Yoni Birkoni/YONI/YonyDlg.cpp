...
//timer of enemys/player picture change/moving
{
	...
	//----------------------------- enemys section
		tis1++;if (tis1>=ctis1)
		{tis1=0;is1+=30;if (is1>30) is1=0;}//enemy picture change
	for (i=0;i<nenemys;i++)
	{
		...
		else if (enemy[i].level < 70) putimagetoscreen(enemy[i].bx, enemy[i].by, enemy[i].bx + 60, enemy[i].by + 60, \
			enemy[i].bx, enemy[i].by, bi, 1);
		else putimagetoscreen(enemy[i].bx, enemy[i].by, enemy[i].bx + 90, enemy[i].by + 90, \
			enemy[i].bx, enemy[i].by, bi, 1);

		//save enemy old position
		enemy[i].bx = enemy[i].x; enemy[i].by = enemy[i].y;
		...
		if ((enemy[i].level>=20))
		{
			if(enemy[i].stat==0)//if master in wait queue, decrease wait queue counter
			{
				...
				{
					if (nenemys<maxenemys-1) enemy[i].stat=-1;
					enemy[i].nstat=0;
				}
			}
			else 
			{//enemy birth faze
				...
				if ((enemy[i].nstat==15)&& (tis1%2==0))
				{
					...
					int mastx=enemy[i].x+15;int masty=enemy[i].y+30;
					mastx-=((mastx-10)%30);masty-=(masty%30);
				...
				if (enemy[i].nstat>25)
				{//if enemy birth faze is ended - start wait faze
					//start wait faze
					if (enemy[i].level<70) enemy[i].nstat=boss;
						else enemy[i].nstat=mast;
					enemy[i].stat=0;
				}
			}
		}
		...
		if (enemy[i].stat <= 0)
		{
			//if monser status>0, death or wait...
				//--------position change
			int n = 0;
			int x1 = (enemy[i].x - 10) / 30; int y1 = (enemy[i].y - 30) / 30;
			...
			if (collisionX && collisionY && !pdeath && !nimb) //collision
				...
				{//enemy death if pchang and enemy level <20
					playbuf(s[8], false);
			...
			//OLD COLLISION REALISATION
			//

			/*
			if ((enemy[i].level != 6) && (!pdeath) && (nimb == 0)) //packman was eaten
			{
				

				if (enemy[i].level >= 70)
				{
					if (((x1 == tmx) && (y1 == tmy)) || ((x1 == tmx + kmx) && (y1 == tmy))\
						|| ((x1 == tmx) && (y1 == tmy + kmy)) || ((x1 == tmx + kmx) && (y1 == tmy + kmy)) || \
						((x1 + 1 == tmx) && (y1 == tmy)) || ((x1 == tmx) && (y1 + 1 == tmy)) || ((x1 + 1 == tmx) && (y1 + 1 == tmy)) || \
						((x1 + 1 == tmx) && (y1 == tmy + kmy)) || ((x1 + 2 == tmx) && (y1 == tmy + kmy)) || \
						((x1 + 2 == tmx) && (y1 == tmy)) || ((x1 + 2 == tmx) && (y1 + 1 == tmy)) || \
						((x1 + 2 == tmx) && (y1 + 2 == tmy)) || ((x1 + 1 == tmx) && (y1 + 2 == tmy)) || \
						((x1 == tmx + kmx) && (y1 + 2 == tmy)))
					{
						ctis /= 3;
						pdeath = true;
				...
				{
					if (((x1 == tmx) && (y1 == tmy)) || ((x1 == tmx + kmx) && (y1 == tmy))\
						|| ((x1 == tmx) && (y1 == tmy + kmy)) || ((x1 == tmx + kmx) && (y1 == tmy + kmy)) && (enemy[i].stat == 0))
					{
						if (!pchang)
						{
							ctis /= 3;
			...
			//monter eater 6 was eaten by big player
			else
			{
				if (((x1 == tmx) && (y1 == tmy)) || ((x1 == tmx + kmx) && (y1 == tmy))\
					|| ((x1 == tmx) && (y1 == tmy + kmy)) || ((x1 == tmx + kmx) && (y1 == tmy + kmy)) || \
...
void hiscore(int scor)
{
	...
	if (fl.Open(hisctmp,CFile::modeRead))
	{
		...
		{//read and convert file data
			strncpy(a[i],w,strchr(w,'\r')-w);
			a[i][strchr(w,'\r')-w]='\0';
			w=strchr(w,'\r')+2;
			strncpy(d,w,strchr(w,'\r')-w);
			d[strchr(w,'\r')-w]='\0';
			w=strchr(w,'\r')+2;
			b[i]=atoi(d);
			if ((!sadd)&&(scor>b[i])) 
