...
typedef struct {
  ...
  int (*mpg123_format)(mpg123_handle *mh, long rate, int channels,
                       int encodings);
  ...
  void (*mpg123_rates)(const long **list, size_t *number);
     ...
     45) /* api (but not abi) change as of mpg123-1.26.0 */
  int (*mpg123_read)(mpg123_handle *mh, void *outmemory, size_t outmemsize,
                     size_t *done);
#else
  ...
  int (*mpg123_replace_reader_handle)(
      mpg123_handle *mh, MIX_SSIZE_T (*r_read)(void *, void *, size_t),
...
#endif
{
  if (mpg123.loaded == 0) {
    ...
    FUNCTION_LOADER(mpg123_format, int (*)(mpg123_handle *mh, long rate,
                                           int channels, int encodings))
    FUNCTION_LOADER(mpg123_format_none, int (*)(mpg123_handle *mh))
    FUNCTION_LOADER(mpg123_getformat, int (*)(mpg123_handle *mh, long *rate,
                                              int *channels, int *encoding))
    FUNCTION_LOADER(mpg123_init, int (*)(void))
    FUNCTION_LOADER(mpg123_new,
                    mpg123_handle * (*)(const char *decoder, int *error))
    FUNCTION_LOADER(mpg123_open_handle,
                    int (*)(mpg123_handle *mh, void *iohandle))
...
#if (MPG123_API_VERSION >=                                                     \
     45) /* api (but not abi) change as of mpg123-1.26.0 */
    FUNCTION_LOADER(mpg123_read, int (*)(mpg123_handle *mh, void *outmemory,
                                         size_t outmemsize, size_t *done))
#else
    FUNCTION_LOADER(mpg123_read,
                    int (*)(mpg123_handle *mh, unsigned char *outmemory,
                            size_t outmemsize, size_t *done))
#endif
    FUNCTION_LOADER(mpg123_replace_reader_handle,
                    int (*)(mpg123_handle *mh,
                            MIX_SSIZE_T (*r_read)(void *, void *, size_t),
                            off_t (*r_lseek)(void *, off_t, int),
                            void (*cleanup)(void *)))
    FUNCTION_LOADER(mpg123_seek,
                    off_t (*)(mpg123_handle *mh, off_t sampleoff, int whence))
...
#define f(x)                                                                   \
  case x:                                                                      \
    return #x;
    f(MPG123_ENC_UNSIGNED_8) f(MPG123_ENC_UNSIGNED_16) f(MPG123_ENC_SIGNED_8)
        f(MPG123_ENC_SIGNED_16) f(MPG123_ENC_SIGNED_32) f(MPG123_ENC_FLOAT_32)
#undef f
  }
  return "unknown";
}
...
static void *MPG123_CreateFromRW(SDL_RWops *src, int freesrc) {
  ...
  result = mpg123.mpg123_replace_reader_handle(music->handle, rwops_read,
                                               rwops_seek, rwops_cleanup);
  if (result != MPG123_OK) {
    Mix_SetError("mpg123_replace_reader_handle: %s",
                 mpg_err(music->handle, result));
    MPG123_Delete(music);
    return NULL;
  }
  ...
  for (i = 0; i < num_rates; ++i) {
    ...
    const int formats =
        (MPG123_ENC_SIGNED_8 | MPG123_ENC_UNSIGNED_8 | MPG123_ENC_SIGNED_16 |
         MPG123_ENC_UNSIGNED_16 | MPG123_ENC_SIGNED_32 | MPG123_ENC_FLOAT_32);
  ...
  music->stream = SDL_NewAudioStream((SDL_AudioFormat)format, (Uint8)channels,
                                     (int)rate, music_spec.format,
                                     music_spec.channels, music_spec.freq);
  if (!music->stream) {
    MPG123_Delete(music);
    return NULL;
  }
...
static int MPG123_GetSome(void *context, void *data, int bytes,
                          SDL_bool *done) {
  MPG123_Music *music = (MPG123_Music *)context;
  int filled, result;
  size_t amount = 0;
  ...
  result = mpg123.mpg123_read(music->handle, music->buffer, music->buffer_size,
                              &amount);
  switch (result) {
  case MPG123_OK:
  ...
  case MPG123_NEW_FORMAT:
    result =
        mpg123.mpg123_getformat(music->handle, &rate, &channels, &encoding);
    if (result != MPG123_OK) {
      return Mix_SetError("mpg123_getformat: %s",
                          mpg_err(music->handle, result));
    }
#ifdef DEBUG_MPG123
    printf("MPG123 format: %s, channels: %d, rate: %ld\n",
...
static int MPG123_GetAudio(void *context, void *data, int bytes) {
  MPG123_Music *music = (MPG123_Music *)context;
  return music_pcm_getaudio(context, data, bytes, music->volume,
                            MPG123_GetSome);
}
...
static int MPG123_Seek(void *context, double secs) {
  ...
  if ((offset = mpg123.mpg123_seek(music->handle, offset, SEEK_SET)) < 0) {
    return Mix_SetError("mpg123_seek: %s",
                        mpg_err(music->handle, (int)-offset));
  }
  return 0;
}
...
static double MPG123_Tell(void *context) {
  ...
  if ((offset = mpg123.mpg123_tell(music->handle)) < 0) {
    return Mix_SetError("mpg123_tell: %s",
                        mpg_err(music->handle, (int)-offset));
  }
...
static void MPG123_Close(void) { mpg123.mpg123_exit(); }

Mix_MusicInterface Mix_MusicInterface_MPG123 = {"MPG123",
                                                MIX_MUSIC_MPG123,
                                                MUS_MP3,
                                                SDL_FALSE,
                                                SDL_FALSE,
