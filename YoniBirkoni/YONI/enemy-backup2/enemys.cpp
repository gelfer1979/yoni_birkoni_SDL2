...
bool ChooseEnemyDirection(int i) {
    ...
    if (enemy[i].level == 6) {
        ...
        if (enemy[i].go < 3) { // ==1,2 (horizontal)
            ...
            if (!IsWallInRect(y1 - 1, x1, 1, cell_w)) n += 1;
            if (!IsWallInRect(y1 + cell_h, x1, 1, cell_w)) n += 2;

            if (enemy[i].go == 1 && IsWallInRect(y1, x1 - 1, cell_h, 1)) {
                if (n == 3) enemy[i].go = mrand(2) + 3;
                else enemy[i].go = n + 2;
            } else if (enemy[i].go == 2 && IsWallInRect(y1, x1 + cell_w, cell_h, 1)) {
                if (n == 3) enemy[i].go = mrand(2) + 3;
            ...
            if (!IsWallInRect(y1, x1 - 1, cell_h, 1)) n += 1;
            if (!IsWallInRect(y1, x1 + cell_w, cell_h, 1)) n += 2;

            if (enemy[i].go == 3 && IsWallInRect(y1 - 1, x1, 1, cell_w)) {
                if (n == 3) enemy[i].go = mrand(2) + 1;
                else if (n > 0) enemy[i].go = n;
                else enemy[i].go = 4;
            } else if (enemy[i].go == 4 && IsWallInRect(y1 + cell_h, x1, 1, cell_w)) {
                if (n == 3) enemy[i].go = mrand(2) + 1;
                else if (n > 0) enemy[i].go = n;
                else enemy[i].go = 3;
            } else {
