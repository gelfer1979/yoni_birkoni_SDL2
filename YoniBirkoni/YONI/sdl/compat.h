...
// Windows types stubs for non-Windows platforms
  ...
  extern bool g_gamepad_virtual_keys[256];
...
inline short GetAsyncKeyState_Gamepad(int vKey) {
    if (vKey >= 0 && vKey < 256 && g_gamepad_virtual_keys[vKey]) {
        return (short)0x8000;
    }
#ifdef _WIN32
...
#endif
#define GetAsyncKeyState GetAsyncKeyState_Gamepad
